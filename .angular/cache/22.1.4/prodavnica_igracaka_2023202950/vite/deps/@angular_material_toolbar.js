...
var MatToolbarRow = class MatToolbarRow {
...
var MatToolbar = class MatToolbar {
	...
	_checkToolbarMixedModes() {
		if (this._toolbarRows.length && (typeof ngDevMode === "undefined" || ngDevMode)) {
			if (Array.from(this._elementRef.nativeElement.childNodes).filter((node) => !(node.classList && node.classList.contains("mat-toolbar-row"))).filter((node) => node.nodeType !== (this._document ? this._document.COMMENT_NODE : 8)).some((node) => !!(node.textContent && node.textContent.trim()))) throwToolbarMixedModesError();
		}
	}
	...
	static ɵcmp = /* @__PURE__ */ ɵɵdefineComponent({
		...
		contentQueries: function MatToolbar_ContentQueries(rf, ctx, dirIndex) {
			if (rf & 1) ɵɵcontentQuery(dirIndex, MatToolbarRow, 5);
			if (rf & 2) {
				let _t;
...
var MatToolbarModule = class MatToolbarModule {
